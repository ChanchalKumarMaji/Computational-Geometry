...
// Handles single points also, but not needed in our Assignment as length of all the given segments are non-zero. 
int solve(vector<vi>& P){
    int n = P.size();
    vector<vi> A;
    map<vi, int> single_points;
    map<vi, int> :: iterator it;
    for(int i=0; i<n; i++){
        int p1x, p1y, p2x, p2y;
        p1x = P[i][0]; p1y = P[i][1]; p2x = P[i][2]; p2y = P[i][3];
        if(p1x == p2x && p1y == p2y)
            single_points[vi({p1x, p1y})] += 1;
        else if(p1x == p2x){
    ...
    int res = 0;
    vector<vi> S;
    for(it = single_points.begin(); it != single_points.end(); it++){
        if(it->second == 1){
    ...
    while(i < A.size()){
        x = A[i][0];
        while(p < S.size() && x > S[p][0]) p++;
        if(A[i][1] == 0){
            int l = A[i][2], h = A[i+1][2];
            //cout << l << " " << h << endl;
            i += 2;
            res += sumRange(l, h);
            while(p < S.size() && x == S[p][0] && S[p][1] <= h){
                if(S[p][1] >= l && S[p][1] <= h){
                    res += 1;
                    visited[p] = 1;
                }
    ...
    for(int j=0; j<S.size(); j++){
        if(!visited[j]){
            int p1x, p1y, p2x, p2y;
            p1x = S[j][0]; p1y = S[j][1]; p2x = S[j][0]; p2y = S[j][1];
            A.push_back(vi({p1x, 0, p1y}));
            A.push_back(vi({p2x, 0, p2y}));
        }
    }
